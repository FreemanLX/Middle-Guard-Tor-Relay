...
catcherr(){
   case $1 in
   0)
       echo "Linux update failed. Please retry later."
	   ;;
   1)
       echo "Unable to install unattended upgrades. Please retry later."
       ;;	   
   esac	   
   exit	   
}
...
verifydepedencies(){
	...
	if ! which $REQUIRED_PKG > /dev/null; then
	  ...
	  if(($? != 0)); then 
	    echo "Couldn't install $REQUIRED_PKG."
	    exit
      fi
      else
...
echo '\nStep 1: Installing necessary depedencies for TOR and Nyx installation'
...
echo 'Step 2: Enabling and configuring automatic software updates'
...
echo 'Step 3: Configuring repository for TOR installation'
...
echo 'Step 4: Installing and configuring TOR '
verifydepedencies "tor"
...
echo 'Step 5: Enabling TOR service and installing Nyx'
